namespace StoryQ.Formatting.Methods
{
    ...
    /// cref="ParametersInlineMethodFormatAttribute" />, and <see
    /// cref="ParameterSuffixedMethodFormatAttribute" />
    /// </summary>
    ...
    public abstract class MethodFormatAttribute : Attribute
    {
        ...
        protected static string UnCamel(string camelText) =>
            CommonStringReplacements.Aggregate(
                $" {camelText.UnCamel()} ",
                (current, p) => current.Replace(p.Key, p.Value, StringComparison.Ordinal)).Trim();
    }
}
